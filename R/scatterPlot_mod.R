...
scatterPlotInputsUI <- function(id, data, title = NULL, columns = 2) {
    ...
    num.choices <- c("", choices[Filter(function(x) is.numeric(x), data)])

    # Get categorical variables of data. 
    cat.choices <- choices[Filter(function(x) !is.numeric(x), data)]

    # Create list of Shiny inputs for all scatterPlot parameters, broken up by sensible categories (e.g. "Data", "Point Styling")
    inputs <- list(
        "Data" = tagList(
            selectInput(ns("x.by"), "X-axis", choices = choices),
            selectInput(ns("y.by"), "Y-axis", choices = choices),
            ...
            selectInput(ns("x.adjustment"), "X-axis adjustment", choices = c("", "z-score", "relative.to.max")),
            selectInput(ns("y.adjustment"), "Y-axis adjustment", choices = c("", "z-score", "relative.to.max")),
            selectInput(ns("color.adjustment"), "Color adjustment", choices = c("", "z-score", "relative.to.max")),
            selectInput(ns("x.adj.fxn"), "X-axis adjustment function", choices = c("", "log2", "log", "log10", "log1p", "as.factor")),
            selectInput(ns("y.adj.fxn"), "Y-axis adjustment function", choices = c("", "log2", "log", "log10", "log1p", "as.factor")),
            selectInput(ns("color.adj.fxn"), "Color adjustment function", choices = c("", "log2", "log", "log10", "log1p", "as.factor")),
        ),
        "Points" = tagList(
            numericInput(ns("size"), "Point size", value = 1),
            checkboxInput(ns("show.others"), "Show others", value = TRUE),
            numericInput(ns("opacity"), "Point opacity", value = 1),
            selectInput(ns("rename.color.groups"), "Rename color groups", choices = cat.choices),
            selectInput(ns("rename.shape.groups"), "Rename shape groups", choices = cat.choices)
        ),
        "Colors" = tagList(
            ...
            selectInput(ns("contour.linetype"), "Contour linetype", choices = c("solid", "dashed", "dotted", "dotdash", "longdash", "twodash"))
        ),
        "Facets" = tagList(
            numericInput(ns("split.nrow"), "Split nrow", value = NULL),
            numericInput(ns("split.ncol"), "Split ncol", value = NULL),
            selectInput(ns("multivar.split.dir"), "Multivar split dir", choices = c("row", "col")),
        ...
        "Labels" = tagList(
            ...
            sliderInput(ns("labels.size"), "Labels size", min = 1, value = 5)
        ),
        "Legend/Scale" = tagList(
            ...
            numericInput(ns("legend.color.size"), "Legend size", min = 1, value = 5),
        ...
        "Extras" = tagList(
            ...
            selectInput(ns("hover.data"), "Hover data", choices = choices, multiple = TRUE),
            numericInput(ns("hover.round.digits"), "Hover round digits", value = 5)
        )
